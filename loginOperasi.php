...
if(isset($_POST["btn_username"])) {
    $qry = mysqli_query($con, "SELECT username FROM users WHERE username='".mres($con,$_POST["username"])."'AND jawatan = 'Operasi'") or die (mysqli_error($con));
    ...
    $qry = mysqli_query($con, "SELECT password FROM users WHERE username='".mres($con,$_POST["username"])."' and password='".md5(mres($con,$_POST["password"]))."' AND jawatan = 'Operasi'") or die (mysqli_error($con));
    if(mysqli_num_rows($qry)>0){
